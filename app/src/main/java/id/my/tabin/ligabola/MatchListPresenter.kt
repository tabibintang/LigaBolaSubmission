...
class MatchListPresenter(
            ...
            for(i in 0 until data.events.size){
                ...
                matchesList.add(
                    Event(
                        ...
                        if(data.events[i].homeScore == null){"-"} else{data.events[i].homeScore},
                        if(data.events[i].awayScore == null){"-"} else{data.events[i].awayScore},
    ...
    fun getMatchSearchList(search: String?) {
        ...
        doAsync {
            val data = gson.fromJson(
                apiRepository
                    .doRequest(TheSportDBApi.getSearchMatch(search)),
                EventSearchResponse::class.java
            )
            for(i in 0 until data.event.size){
                val dataHome = gson.fromJson(
                    apiRepository
                        .doRequest(TheSportDBApi.getTeamDetail(data.event[i].homeTeam)),
                    TeamResponse::class.java
                )
                val dataAway = gson.fromJson(
                    apiRepository
                        .doRequest(TheSportDBApi.getTeamDetail(data.event[i].awayTeam)),
                    TeamResponse::class.java
                )
                matchesList.add(
                    Event(
                        ...
                        if(data.event[i].homeScore == null){"-"} else{data.event[i].homeScore},
                        if(data.event[i].awayScore == null){"-"} else{data.event[i].awayScore},
